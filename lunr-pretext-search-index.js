...
},
{
  ...
  "body": " Lab: AC to DC Conversion Using Diode Bridge Rectifier   Objective: In your lab notebook, write a few sentences stating your objective in conducting this laboratory exrercise. Consider the following questions:   What kind of circuit(s) or components are you exploring?    What is new about this circuit as compared to circuits previously studied?    What theoretical predictions do you have regarding circuit behavior?    What effects do you expect to observe?   Your answers should be specific to the type of circuit you are examining, but do not discuss specific component values.   Equipment: Proto-board, digital oscilloscope, DMM, INA117P differential amplifier chip, 4 x 1N4001 silicon diodes, 3.3k resistor, five capacitors in the range from 1 F to 500 F.   Introduction   The operation of many electronic devices depend on a DC voltage source, but the electricity delivered to households in the United States is 60-Hz AC voltage. Today, we'll explore one method of converting AC voltage to DC voltage using the bridge full-wave rectifier with a smoothing capacitor as pictured in .    This circuit converts an AC input voltage to a nearly-constant output voltage, though a small ripple will remain. Larger values of smoothing capacitance will reduce the size of this ripple and thus will improve the quality of the DC voltage output. High-capacitance capacitors are typically more expensive and physically larger than small capacitors, so there is incentive to keep the capacitance as small as possible. Thus, devices that require DC voltages often specify a maximum ripple size that they can tolerate.  Your goal in this experiment is to predict the optimal smoothing capacitance one would use to ensure a 5% ripple on . The ripple size depends on the smoothing capacitance (which you will determine), the input signal frequency (which we will assume to be 120-Hz) and the load resistance provided by the device across the output (here, represented by the 3.3-k resistor).  Before conducting the experiment, be sure to review the behavior of the unsmoothed bridge full-wave rectifier circuit in .   NOTE: One downside of the circuit shown in is that neither terminal is connected to ground. This means that we cannot simply connect a single oscilloscope probe across since the ground provided by the oscilloscope and the ground provided by the function generator would conflict. We will use a differential amplifier chip based on operational amplifiers that takes and as inputs and produces a single output signal referenced to ground. The details of this differential amplifier's operation and the behavior of operational amplifiers in general will be treated later.    EXPERIMENT   Once you've reviewed the above information, complete the following steps.    Use your DMM to determine exact values for your resistor and your capacitors.   Warning: The capacitors that you are using are called electrolytic capacitors and are polarized, meaning that they only work when the negative lead (often marked with a negative sign) is at a lower voltage than the positive lead. When measuring the capacitance with the DMM, connect the COM to the negative capacitor lead.    Plug in your prototyping board but keep it turned off for the moment.    Set up a DMM to measure the variable V+ (yellow) voltage with respect to ground. Pay attention to which of the four top rows correspond to +V and Ground. Turn on the prototyping board.    Adjust the appropriate prototyping board knob so that the +V strip is +15V relative to Ground. Repeat the process above to set -V to be -15V relative to Ground. Do not touch these knobs for the rest of this lab session.    Turn off your prototyping board.    Construct the circuit shown in on your prototyping board. Use the smallest capacitor value that you've been given as your smoothing capacitor. The input voltage will be provided by your function generator.   Warning: Pay attention to the capacitor polarity in the circuit schematic. If you wire the capacitor in backward, you can destroy the capacitor (sometimes resulting in a puff of smoke or small flame). Remember that always in this circuit.     Connect the V supplies to the INA117P differential amplifier chip as shown in . Note that your instructor should have already inserted the INA117P chip into your prototyping board in the correct orientation.   Differential amplifier setup.   (for accessibility)      Connect points and from your diode bridge rectifier circuit to the differential amplifier chip as shown in     Set up the oscilloscope to measure on CH1 and  from the differential amplifier on CH2. Turn the oscilloscope on.    Turn on the function generator (but leave the OUTPUT disabled). Set the generator to produce a 120-Hz sinusoidal voltage with a peak amplitude of 5-V. Then, enable the OUTPUT.    Adjust the CH1 vertical scale so that the input signal is as large as possible without cutting off any of the signal. Adjust the horizontal timebase such that you can see 3-5 periods of the sinusoidal signal.    Adjust the CH2 vertical scale to be identical to the CH1 vertical scale from the previous step.    Use the CURSOR functionality to measure the amplitude of .   Q: Record your amplitude in your lab notebook.   P: Insert into your lab notebook a photograph of the oscilloscope screen displaying and .    Now, turn off the display of on the oscilloscope as we're going to focus the rest of our investigation on .    Enter the CH2 menu and choose AC coupling. This will remove the DC offset and allow you to magnify the AC ripple only.    Repeat the following steps for each smoothing capacitor you've been given:   Swap out the smoothing capacitor for the next largest capacitor (unless this is the first capacitor you're testing). Make sure to turn off the prototyping board while you are swapping out capacitors and that you've oriented the polarized capacitor correctly. Turn the prototyping board back on once the new capacitor is in place.    Adjust the CH2 vertical scale to maximize the size of the AC ripple signal and use the cursor functionality to measure the amplitude of the AC ripple.   Q: Record the ripple size and smoothing capacitance in your lab notebook.       Disable the function generator OUTPUT and turn off your oscilloscope.    Use Python to do the following:   Perform a curve fit to your vs data. Assume a function of the form where and are fitting parameters and is your capacitance variable.    Plot ripple voltage size vs smoothing capacitance and use symbols to display your data points. Overplot a line representing your curve fit.      P: Include your plot in your lab notebook.   Q: Provide your fit parameters in your lab notebook.    Predict the minimum smoothing capacitance one would require to ensure a 5% ripple on .   Q: Include this result in your lab notebook.    Provide a discussion in your lab notebook based on standard lab notebook guidelines.   "
},
{
