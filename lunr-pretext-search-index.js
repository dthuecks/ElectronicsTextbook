...
},
{
  ...
  "body": " Newton's Method  Newton's method is a computational algorithm that can be used to find the roots of an equation. Let's illustrate with an example.      In , the solid blue line represents the function . Assume that is a root of such that . If we take a Taylor expansion of at , we find that . for close to the real function root, we can assume that the terms of the expansion containing second-order derivatives and above are very small and can be neglected. This means that or . So, in order to computationally determine the root of a function, we make an initial guess of the root's value and use to generate a new and better estimate of the function's root . The closer is to the real root , the better our estimate of the root will be using . As such, we will make an initial guess , find an estimate of the root , and iterate, using as our new guess to make a further improved estimate of the root. This iteration continues until the change in the estimated value of the root changes by less than some tolerance that is set by the user. This method requires that the user know the functional form of and can evaluate the first derivative .  "
},
{
